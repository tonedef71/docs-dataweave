...
DataWeave supports two types of collections: Arrays and Objects. Arrays are a sequence of values, while Objects are a sequence of key-value pair. 
...
This example builds a filter function that is higher-order function:
...
To match over an object, use the following syntax, where the script gets the first value of an object in the case "Word":
